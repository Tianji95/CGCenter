...
namespace Zxen {
    static ZXEngine* engine = nullptr;
    ...
    void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
    {
        ...
        if (engine == nullptr || engine->GetScene() == nullptr || engine->GetScene()->GetCamera() == nullptr) {
            return;
        }
        CameraPtr camera = engine->GetScene()->GetCamera();
        switch (key) {
    ...
    bool UIFramework::Init()
    {
        ...
        window = glfwCreateWindow(1280, 720, "ZXEngine", NULL, NULL);
        if (window == NULL)
            return false;
        ...
        engine = new ZXEngine();
        Render* render = engine->CreateRender<ForwardRender>();
        Scene* scene = engine->CreateScene();
        ResourceManager* rsm = engine->CreateResourceManager();
        rsm->LoadDataToScene(scene, "F:/GitHub/CGCenter/3dmodels/home/home.obj", "F:/GitHub/CGCenter/3dmodels/skybox/");
    ...
    bool UIFramework::Draw()
    {
        ...
        Render* render = engine->GetRender();
        {
            ...
            ImGui::ColorEdit3("light Ambient color", (float*)&render->ambientColor);
            ImGui::ColorEdit3("light diffuse color", (float*)&render->diffuseColor);
            ImGui::ColorEdit3("light specular color", (float*)&render->specularColor);
            ImGui::SliderFloat("light constant", &render->cons, 0.1f, 10.0f);
            ImGui::SliderFloat("light linear", &render->lin, 0.0f, 0.001f, "%.6f");
            ImGui::SliderFloat("light quadratic", &render->quad, 0.0f, 0.000001f, "%.9f");
            ImGui::SliderFloat("light cutOff", &render->cutOff, 30.0f, 150.0f);
            ImGui::SliderFloat("light outerCutOff", &render->outerCutOff, 30.0f, 150.0f);
    ...
    {
        engine->DestoryRender();
        engine->DestoryResourceManager();
        engine->DestoryScene();
        delete engine;
        // Cleanup
